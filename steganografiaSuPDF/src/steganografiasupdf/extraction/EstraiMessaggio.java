    ...
    private void estraiMessaggio() {
        try {
            ...
            for(int i = 1; i <= numPagine; i++){
                numPagina++;
                this.listaSottoseq.removeAll(listaSottoseq);
                estraiMessaggioPagina(i, reader);
                ...
                if(listaSottoseq.isEmpty()){
                    ...
                    //this.steganografia.getSp().getsEstrazione().mostraMessaggio(contaImmagini);
                    MessEstrattoPerPagina mepp = new MessEstrattoPerPagina(numPagina, mess);
                    listaMessEstratti.add(mepp);
                }
                
            }
            //estraiMessaggioPagina(1, reader);
            reader.close();
            log.info("Dimensione listaMessEstratti: " + listaMessEstratti.size());
            for(int i = 0; i < listaMessEstratti.size(); i++){
                int numPagina = listaMessEstratti.get(i).getNumPagina();
                String messEstratto = listaMessEstratti.get(i).getMessEstratto();
                log.info("Pagina " + numPagina + "\nMessaggio Estratto: " + messEstratto);
            }
            this.steganografia.getSp().getsEstrazione().mostraMessaggio(listaMessEstratti);
